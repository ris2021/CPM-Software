...
begin
  clrscr;
  write(#30'SOKOBAN for CRISS CP/M, Level: ',lvl);
  gotoxy(74,1);writeln('v.1.2');
  for y:=1 to 23 do begin
    ...
    for x:=1 to length(f[y]) do begin
      ...
      case f[y][x] of
        ...
        '@':write(#$f4#$f5);
        'M','m':begin write(#250#251);mx:=x;my:=y;end;
      end;
    end;
...
begin
  ...
  repeat
    ...
    bc:=0;wscr;
    repeat
      gotoxy(40,1);write('Boxes: ',bc,'  Moves: ',mv:1:0,'    ');
      if bios(1)=255 then begin
        c:=upcase(chr(bios(2)));
        if msb<>-1 then msb:=1;
      end else begin
        c:=#0;
